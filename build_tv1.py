...
def build():
    ...
    # 술/음료 광고 이미지 로드 (술광고등/ 폴더)
    ad_images = []
    if os.path.isdir(ADS_DIR):
        ad_images = sorted([f for f in os.listdir(ADS_DIR)
                            if f.lower().endswith(('.jpg', '.jpeg', '.png', '.webp'))])
    ...
    # 모든 광고/명언이 HTML 에 들어가도록 자리당 장수 계산 (1회 루프로 전부 노출)
    AD_PER_SLOT = max(1, _math.ceil(len(ad_images) / max(1, est_slots))) if ad_images else 0
    QUOTE_BATCH = max(1, _math.ceil(len(quote_queue) / max(1, est_slots))) if quote_queue else 0
    print(f"추정 자리 수: {est_slots} | 자리당 술광고: {AD_PER_SLOT}장 | 자리당 명언: {QUOTE_BATCH}장")
    ...
    def make_logo_slide():
        """풀스크린 로고 슬라이드 (tv3 스타일) - 흰 배경 + 할프톤 + 바운싱 로고"""
        if not tv3_logo_b64:
            return None
        return (
            f'<div class="slide slide-logo">'
            f'<div class="logo-halftone"></div>'
            f'<img class="logo-main" src="{tv3_logo_b64}" alt="JooNs Sushi">'
            f'</div>'
        )

    def pop_slot_slides():
        """자리 1개 = 프로모션 + 술광고 + 명언 모아 자리 안에서 랜덤 셔플 + 로고 1장 앞에 배치"""
        bucket = []
        for _ in range(PROMO_PER_SLOT):
            if promo_queue:
                bucket.append(make_promo_img_slide(promo_queue.pop(0)))
        for _ in range(AD_PER_SLOT):
            if ad_queue:
                bucket.append(make_ad_img_slide(ad_queue.pop(0)))
        for _ in range(QUOTE_BATCH):
            if quote_queue:
                bucket.append(make_quote_img_slide(quote_queue.pop(0)))
        random.shuffle(bucket)
        # 자리마다 로고 1장 선두에 고정 (중간중간 브랜딩)
        logo = make_logo_slide()
        if logo:
            bucket.insert(0, logo)
        return bucket
    ...
    # 로고 (reference/logo.png) - 커버 메인
    logo_b64 = to_b64(os.path.join("reference", "logo.png")) if os.path.exists(os.path.join("reference", "logo.png")) else ""
    # 풀스크린 로고 슬라이드용 (tv3 의 joon_logo.png) - 중간중간 삽입
    tv3_logo_path = os.path.join("..", "joon_tv3", "joon_logo.png")
    tv3_logo_b64 = to_b64(tv3_logo_path) if os.path.exists(tv3_logo_path) else ""

    cover_logo_html = (
        f'<div class="cover-logo"><img src="{logo_b64}" alt="JooNs Sushi"></div>'
        if logo_b64 else
        '<div class="cover-title">JooN\'s Sushi</div>'
    )
    slides.append(
        f'<div class="slide slide-title slide-cover active" data-accent="#C9A96E" data-atmos="#060609"'
...
/* 커버 로고 */
.cover-logo{display:flex;justify-content:center;align-items:center;margin-bottom:18px;}
.cover-logo img{max-width:720px;width:55vw;height:auto;filter:drop-shadow(0 6px 30px rgba(201,169,110,0.35)) drop-shadow(0 2px 12px rgba(0,0,0,0.8));animation:logoFloat 4.5s ease-in-out infinite;}
@keyframes logoFloat{0%,100%{transform:translateY(0);}50%{transform:translateY(-8px);}}
@keyframes logoIntro{from{opacity:0;transform:scale(0.85);}to{opacity:1;transform:scale(1);}}
.slide-cover.active .cover-logo img{animation:logoIntro 1.2s ease-out, logoFloat 4.5s ease-in-out 1.2s infinite;}

/* 풀스크린 로고 슬라이드 (tv3 스타일) — 중간중간 삽입 */
.slide-logo{background:#FFF;padding:0;}
.slide-logo .logo-halftone{position:absolute;inset:0;background-image:radial-gradient(circle,#222 1.5px,transparent 1.6px),radial-gradient(circle,#222 1.5px,transparent 1.6px);background-size:18px 18px,18px 18px;background-position:0 0,9px 9px;opacity:0.10;}
.slide-logo .logo-main{position:relative;z-index:5;max-width:70vw;max-height:65vh;width:auto;height:auto;mix-blend-mode:multiply;}
@keyframes logoBounce{from{transform:scale(0.97) rotate(-1deg);}to{transform:scale(1.03) rotate(1deg);}}
.slide-logo.active .logo-main{animation:logoBounce 2s ease-in-out infinite alternate;}
...
/* 이미지 명언 슬라이드 - TV overscan safe area + 완화된 Ken Burns */
.slide-quote-img{background:#000;padding:0;}
.qimg-bg{position:absolute;inset:3vh 3vw;background-size:contain;background-position:center;background-repeat:no-repeat;}
.slide-quote-img.active .qimg-bg{animation:qKenBurns 10s ease-out forwards;}
@keyframes qKenBurns{0%{transform:scale(0.98);}100%{transform:scale(1.0);}}
...
tick();
    ...
    total_promo = sum(1 for s in slides if 'slide-promo-img' in s)
    print(f"v8.0+ads Build OK | 명언:{total_extra} | 메뉴:{total_menu} | 술광고:{total_ad} | 프로모션:{total_promo} | 총:{len(slides)} 슬라이드")
